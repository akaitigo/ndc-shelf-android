...
`app/schemas/dev.ndcshelf.app.data.local.AppDatabase/<version>.json` を履歴の正本としてGit管理する。現在はv9で、必須列、主キー、外部キー、unique index、旧置き場所文字列、棚内順序、シリーズ所属と確定由来の値保持を `AppDatabaseMigrationTest` が検証する。
