...
## Status（2026-08-01 追記）

**実装済み。ただし配布物には含めない。**

Pixel 7での実機検証により、次が判明した（実測値は `docs/ON_DEVICE_LLM_FINDINGS.md`）。

- 全27冊を書誌JSONで渡すとcontextを超えて即座に失敗する（配布元artifactのcontextは2,048で、本ADR記載の8,192は誤り）
- 1冊1行へ圧縮すれば7.6秒で妥当な回答が返る。**冊数ではなく1冊あたりの情報量が問題だった**
- Tensor G2はOpenCLを公開しておらずGPU経路を使えない
- peak RSS 2.3 GBで、推論中に他アプリがOOMで終了する
- ランタイムの `ResponseFormat` / `ThinkingConfig` / tool calling を使っていない

475 MBのモデル取得を促しておいて回答が返らない体験になるため、`ai`フレーバーは
ビルドとサイズ検証だけ継続し、**GitHub Releasesへは添付しない**。
再設計の方針は上記ドキュメントに残した。
