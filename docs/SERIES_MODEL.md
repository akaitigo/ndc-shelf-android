...
v7からv8は2テーブルとindexだけを追加し、既存Work・Edition・Copyを更新しません。v8からv9はMembershipへ確定由来3列を追加し、既存行を`MANUAL`、`USER`、空の原題として保持します。完全バックアップ形式もv9へ上げ、由来enumと原題を検証・復元します。v1からv7はSeriesなし、v8は既存Membershipを手動確定として引き続き読めます。
