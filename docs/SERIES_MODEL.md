...
- `SeriesSuggestion`は未所属Workのタイトルから生成する一時データで、Roomやバックアップへ保存しない。
- 明示的な「第N巻」「N巻」、上下中巻、前後編を高、空白区切りの数字・ローマ数字を中、外伝を低信頼度として表示する。信頼度にかかわらず自動確定しない。
- 利用者は共通シリーズ名、巻ラベル、種別、対象冊、刊行順を編集し、新規シリーズまたは既存シリーズへ一冊ずつ・一括で確定できる。
- 確定時はWorkタイトルが候補生成時の`sourceTitle`と一致することを再確認し、全Membershipを単一transactionで追加する。一件でも古い・重複・不正なら全件を中止する。
- `origin`は`TITLE_SUGGESTION`または`MANUAL`、`confirmedBy`は現在`USER`だけを許可する。解除はMembershipだけを削除し、Work、Edition、OwnedCopy、読書状態を保持する。

## Migrationと互換性

DB v7からv8は2テーブルとindexだけを追加し、既存Work・Edition・Copyを更新しません。v8からv9はMembershipへ確定由来3列を追加し、既存行を`MANUAL`、`USER`、空の原題として保持します。完全バックアップ形式はシリーズ追加時の形式9から形式10へ上げ、由来enumと原題を検証・復元します。形式1から8はSeriesなし、形式9は既存Membershipを手動・ユーザー確定として引き続き読めます。
