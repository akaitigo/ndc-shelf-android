...
Room v15は同期identity、Keystore wrapping keyで暗号化した鍵blob、検証済みdevice registry cache、招待、処理済みenvelope、security quarantineを保持します。`SyncBackend` interfaceがtransport契約（capability、head CAS、content-addressed object、registry、device envelope、remote削除）を定義し、参照実装の`FolderSyncBackend`は利用者が選んだフォルダ（SAF）へE2EE ciphertextだけを保存します。TLS・認証・期限切れ・rate limit・サービス停止・権限喪失・容量不足は`SyncBackendErrorKind`で分類し、retry可否をenumで判定します。
