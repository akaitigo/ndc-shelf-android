...
- 端末内LLM AI司書の安全基盤（推論エンジンとモデルは未同梱・未登録のため、AI司書の動作は従来どおり規則ベース）。許可済みモデルだけを扱うモデル台帳、HTTPS・許可host・redirect拒否・サイズ上限・SHA-256検証つきの取得経路、検証成功後にだけ有効化する原子的な導入と旧版の保持、API level・CPU種別・メモリ・空き容量による起動前のfail-closed判定、固定指示文とJSON値としての書誌隔離、未知refや過剰件数・不正形式を破棄する厳格な出力検証、失敗時に検証済みの規則ベース回答へ戻す縮退、モデルと派生ファイルの全削除に対応。推論経路はネットワーク機能を使わず、端末内診断へはモデル版・エンジン版・ハッシュ先頭・所要時間・失敗分類だけを記録する。推論runtimeとモデルの採用は配布サイズ予算の承認後に行う。詳細はdocs/adr/0009-on-device-llm-librarian.mdとdocs/LOCAL_LLM_MODELS.md。
