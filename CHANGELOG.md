...
- 英語表示のときに、蔵書カードの所蔵ラベルと置き場所だけ日本語（「所蔵本」「未設定」）のままだった問題を修正した。これらは画面の文言ではなく**データベースへ保存される値**として日本語が埋め込まれており、保存値をそのまま表示していたことが原因だった。あわせて、未設定を選んだときに保存される値が端末の言語によって変わっていた（英語端末では`Not set`が保存されていた）問題も直した。未設定は言語に依存しない値で保存し、表示のたびに端末の言語へ合わせる。既存の蔵書データはアプリ更新時に自動で変換され、利用者が入力した置き場所や所蔵ラベルは変更しない。

### Added

- READMEへ「既知の制約」を追加した。端末内LLMを配布していないこと、複数端末同期が未提供であること、自動更新がないこと、v0.1.2からの更新経路の制約、書誌情報がNDL Searchに依存すること、現在の配布がpre-releaseであることを明記した。

## [0.6.0] - 2026-08-01

**署名付きAPKを配布する最初の版。** 機能は0.5.0と同じで、利用者向けの追加はない。
0.5.0まではタグを作成したのみでAPKを配布していなかった。

### Changed

- AI司書は引き続き端末内の規則ベースで提案する。端末内の言語モデルによる自然文の提案は、実機検証で実用に足りないことが分かったため配布物へ含めていない（`docs/ON_DEVICE_LLM_FINDINGS.md`）。
