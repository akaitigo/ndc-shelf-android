...
- 任意のE2EE同期（既定OFF）。アカウント不要で、利用者が選んだフォルダへ端末鍵で暗号化した同期データだけを保存します。同意と保存先選択を経て有効化し、招待コードと確認コードの照合による端末追加、接続端末の一覧・最終同期表示・個別失効、保存先の全削除と完了確認、同期停止をデータ管理画面から操作できます。定期同期は同意時だけ登録され、同意なし・撤回後は通信・ファイルアクセス・鍵生成を行いません（Room v15）。
