...
class LibraryImportCommitterTest {
    @Test
    fun `stale preview is rejected before writes`() = runBlocking {
        val preview = preview(existing = listOf(book("existing")), additions = listOf(book("new")))
        var writes = 0
        val committer = LibraryImportCommitter(
    ...
    @Test
    fun `partial write failure rolls back the transaction`() = runBlocking {
    ...
    @Test
    fun `cancellation rolls back and is propagated`() = runBlocking {
        val existing = book("existing")
        var stored = listOf(existing)
    ...
    @Test
    fun `successful commit reports preview counts`() = runBlocking {
        val existing = book("existing")
        val addition = book("new")
        var stored = listOf(existing)
        val committer = LibraryImportCommitter(
