...
class DatabaseBackupCodecTest {
    ...
    fun `format seven backup is restored with no inferred series`() {
        ...
        val preview =
            DatabaseBackupCodec(currentDatabaseVersion = 8).decode(
                ByteArrayInputStream(zip(mapOf("manifest.json" to manifest, "database.json" to payload))),
            )
    ...
    @Test
    fun `format nine membership is restored as user confirmed manual data`() {
        val payload =
            """
            {
              "schemaVersion":9,
        ...
        val preview =
            DatabaseBackupCodec(currentDatabaseVersion = 9).decode(
                ByteArrayInputStream(zip(mapOf("manifest.json" to manifest, "database.json" to payload))),
            )
    ...
    @Test
    fun `format twelve backup is restored with zero reading sessions`() {
        val (archive, _) = codec.encode(sampleSnapshot(), "0.4.0", 1)
        val entries = unzip(archive).toMutableMap()
        val payload =
            requireNotNull(entries["database.json"])
        ...
        assertEquals(emptyList<ReadingSessionEntity>(), preview.snapshot.readingSessions)
        assertEquals(0, preview.metadata.readingSessionCount)
        assertEquals(sampleSnapshot().copies, preview.snapshot.copies)
    }

    @Test
    fun `invalid reading sessions are rejected before backup`() {
        val sample = sampleSnapshot()
        val orphan =
            sample.copy(
                readingSessions = sample.readingSessions.map { it.copy(copyId = "missing-copy") },
            )
        val badStatus =
            sample.copy(
                readingSessions = sample.readingSessions.map { it.copy(status = "UNKNOWN") },
            )
        val badDay =
            sample.copy(
                readingSessions = sample.readingSessions.map { it.copy(startedDay = "2026/07/29") },
            )
        val badRating =
            sample.copy(
                readingSessions = sample.readingSessions.map { it.copy(rating = 6) },
            )
        val duplicated =
            sample.copy(
                readingSessions = sample.readingSessions + sample.readingSessions.first(),
            )

        listOf(orphan, badStatus, badDay, badRating, duplicated).forEach { invalid ->
            val error =
                assertThrows(BackupCodecException::class.java) {
