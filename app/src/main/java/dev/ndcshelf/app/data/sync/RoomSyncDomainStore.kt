    ...
    override suspend fun applyUpserts(entities: List<SyncResolvedEntity>): SyncDomainApplyResult {
        ...
        ordered.forEach { entity ->
            when (entity.entityType) {
                "work" -> database.libraryDao().upsertWorks(listOf(entity.toWork()))
                "edition" -> database.libraryDao().upsertEditions(listOf(entity.toEdition()))
                "ownedCopy" -> database.libraryDao().upsertCopies(listOf(entity.toCopy()))
                "wishlistItem" -> database.libraryDao().upsertWishlistItems(listOf(entity.toWishlist()))
                "locationRoom" -> database.locationDao().upsertRooms(listOf(entity.toRoom()))
                "locationShelf" -> database.locationDao().upsertShelves(listOf(entity.toShelf()))
                "locationTier" -> database.locationDao().upsertTiers(listOf(entity.toTier()))
                "series" -> database.seriesDao().upsertSeries(entity.toSeries())
                "seriesMembership" -> database.seriesDao().upsertMemberships(listOf(entity.toSeriesMembership()))
                "workGroup" -> database.workGroupDao().upsertGroups(listOf(entity.toWorkGroup()))
                "workGroupMembership" -> database.workGroupDao().upsertMemberships(
...
private fun SyncResolvedEntity.toWork() = BookWorkEntity(
    id = entityId,
    title = fields.requiredString("title", 1_000),
    primaryAuthor = fields.requiredString("primaryAuthor", 1_000),
)

private fun SyncResolvedEntity.toEdition() = BookEditionEntity(
    id = entityId,
    ...
    classificationSource = fields.requiredEnum(
...
private fun SyncResolvedEntity.toCopy() = OwnedCopyEntity(
    id = entityId,
    ...
    readingStatus = fields.requiredEnum(
        "readingStatus",
...
private fun SyncResolvedEntity.toWishlist() = WishlistItemEntity(
    editionId = entityId,
...
private fun SyncResolvedEntity.toRoom() = LocationRoomEntity(
    id = entityId,
    name = fields.requiredString("name", 500),
    sortOrder = fields.requiredInt("sortOrder"),
)

private fun SyncResolvedEntity.toShelf() = LocationShelfEntity(
    id = entityId,
...
private fun SyncResolvedEntity.toTier() = LocationTierEntity(
    id = entityId,
...
private fun SyncResolvedEntity.toSeries() = SeriesEntity(
    id = entityId,
...
private fun SyncResolvedEntity.toSeriesMembership() = SeriesMembershipEntity(
    id = entityId,
...
private fun SyncResolvedEntity.toWorkGroup() = WorkGroupEntity(
    id = entityId,
    title = fields.requiredString("title", 1_000),
    primaryAuthor = fields.requiredString("primaryAuthor", 1_000),
...
private fun SyncResolvedEntity.toWorkGroupMembership() = WorkGroupMembershipEntity(
    id = entityId,
...
): String = requireNotNull(get(name)?.jsonPrimitive?.contentOrNull)
    .also { value -> require((allowEmpty || value.isNotBlank()) && value.length <= maxLength) }

private fun Map<String, JsonElement>.optionalString(name: String, maxLength: Int): String? {
...
private fun Map<String, JsonElement>.requiredLong(name: String): Long =
    requireNotNull(get(name)?.jsonPrimitive?.longOrNull)

private fun Map<String, JsonElement>.requiredInt(name: String): Int =
    requireNotNull(get(name)?.jsonPrimitive?.intOrNull)
...
private fun Map<String, JsonElement>.requiredBoolean(name: String): Boolean =
    requireNotNull(get(name)?.jsonPrimitive?.booleanOrNull)

private fun Map<String, JsonElement>.requiredEnum(name: String, allowed: Set<String>): String =
    requiredString(name, 100).also { value -> require(value in allowed) }
